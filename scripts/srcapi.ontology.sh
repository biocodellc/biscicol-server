...
var rdfData=fs.readFileSync('../srpdio/srpdio.owl').toString();

var contentType='application/rdf+xml';
var baseUrl="https://github.com/UA-SRC-data";
...
allClasses = classWalker([],'http://purl.obolibrary.org/obo/PATO_0000033','present').sort(sortBy('label'))

// deDuplicate
allClasses = deDuplicate(allClasses,'termID');

// Write long form of JSON files
writeFile("src_data/all.json",allClasses);

// Write short form of JSON files
writeFile("src_data/all_short.json",createShortFile(allClasses))
